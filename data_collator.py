...
def _torch_collate_prompt_batch(prompt_indexes, prompt_lr_label, prompts_length, prompts_lr_length):
    """Collate `examples` into a batch, using the information in `tokenizer` for padding if necessary."""
    import numpy as np
    import torch
    paddings = [prompt_indexe[-1] for prompt_indexe in prompt_indexes]

    max_length = min(prompts_length, max(1, max(len(x) for x in prompt_indexes)-1))
    prompt_indexes = [prompt_index[:-1]+(max_length-len(prompt_index)+1)*[paddings[i]] for i, prompt_index in enumerate(prompt_indexes)]
    left_length = [len(lab[0]) for prompt_lr_lab in prompt_lr_label for lab in prompt_lr_lab]
    right_length = [len(lab[1]) for prompt_lr_lab in prompt_lr_label for lab in prompt_lr_lab]
    max_left_length = max(left_length) if len(left_length) >=1 else 0
    max_right_length = max(right_length) if len(right_length) >=1 else 0
    max_lr_length = min(prompts_lr_length, max(max_left_length, max_right_length))
    cat_prompt_lr_label = []
    for i in range(len(prompt_indexes)):
        if i >= len(prompt_lr_label):
            prompt_lr_label.append([])
        cat_prompt_lr_label.append([])
        for j in range(len(prompt_indexes[i])):
            if j >= len(prompt_lr_label[i]):
                prompt_lr_label[i].append([[], []])
            l = prompt_lr_label[i][j][0] + (max_lr_length - len(prompt_lr_label[i][j][0])) * [-100]
            r = prompt_lr_label[i][j][1] + (max_lr_length - len(prompt_lr_label[i][j][1])) * [-100]
            cat_prompt_lr_label[i].append(l+r)

    # Check if padding is necessary.
        # Tensorize if necessary.
    if isinstance(prompt_indexes[0], (list, tuple, np.ndarray)):
        prompt_indexes = [torch.tensor(e, dtype=torch.long) for e in prompt_indexes]

    if isinstance(cat_prompt_lr_label[0], (list, tuple, np.ndarray)):
        cat_prompt_lr_label = [torch.tensor(e, dtype=torch.long) for e in cat_prompt_lr_label]
    return torch.stack(prompt_indexes, dim=0), torch.stack(cat_prompt_lr_label, dim=0)
...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    return_tensors: str = "pt"
    prompts_lr_length: int = 3
    prompts_length: int = 8
...
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling):
    ...
    # def pad_prompt_lr_mask(self, prompts_lr_char):
    #     for prompt_lr_char in prompts_lr_char:
    #         prompt_lr_char[0] = prompt_lr_char[0]+(self.pao_length-len(prompt_lr_char[0]))*[self.tokenizer.pad_token_id]
    #         prompt_lr_char[1] = prompt_lr_char[1]+(self.pao_length-len(prompt_lr_char[1]))*[self.tokenizer.pad_token_id]
    #     return prompts_lr_char
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        for e in examples:
            ...
            mask_label, char_mask_label, prompt_indexes, lr_prompt_chars = self._whole_word_mask(ref_tokens, e["input_ids"])
            # lr_prompt_chars = self.pad_prompt_lr_mask(lr_prompt_chars)
            mask_labels.append(mask_label)
            char_mask_labels.append(char_mask_label)
            prompts_indexes.append(prompt_indexes)
            lr_prompts_chars.append(lr_prompt_chars)
        #batch_prompts_indexes, batch_lr_prompts_chars = _torch_collate_prompt_batch(prompts_indexes, lr_prompts_chars, self.prompts_length, self.prompts_lr_length)
        # labels[labels == self.tokenizer.pad_token_id] = -100
        batch_mask = _torch_collate_batch(mask_labels, self.tokenizer, pad_to_multiple_of=self.pad_to_multiple_of)
        batch_char_mask = _torch_collate_batch(char_mask_labels, self.tokenizer, pad_to_multiple_of=self.pad_to_multiple_of)
        inputs, labels = self.torch_mask_tokens(batch_input, batch_mask, batch_char_mask)
        #return {"input_ids": inputs, "labels": labels, "prompt_indexs": batch_prompts_indexes, "pao_labels": batch_lr_prompts_chars}
    ...
    def _whole_word_mask(self, input_tokens: List[str], input_ids: List[int], max_predictions=512):
        """
        ...
        i = 0
        cand_ngram_indexes = []
        while i < len(cand_indexes):
            ngram = get_rand_ngram()
            if ngram == 1 and len(cand_indexes[i]) == 1:
                ngram += 1
            cand_ngram_indexes.append([index for indexes in cand_indexes[i: ngram+i] for index in indexes])
            i += ngram

        random.shuffle(cand_ngram_indexes)
        num_to_predict = min(max_predictions, max(1, int(round(len(input_tokens) * self.mlm_probability))))
        masked_lms = []
        mask_length = 0
        prompt_num = 0
        covered_indexes = set()
        char_coverd_indexes = set()
        prompt_indexes = []
        lr_prompts_chars = []
        for index_set in cand_ngram_indexes:
            prompt_index = []
            if prompt_num >= self.prompts_length:
                break
            # index_set = [index for word_indexs in index_set for index in word_indexs]
            if mask_length >= num_to_predict:
                break
            # If adding a whole-word mask would exceed the maximum number of
            # predictions, then just skip this candidate.
            rands = random.sample(range(len(index_set)), max(1, len(index_set)//3)) if len(index_set) > 1 else []
            if mask_length + len(index_set)-len(rands) > num_to_predict:
                continue
            ...
            if is_any_index_covered:
                continue
            # rands = random.randint(0, len(index_set))
            left_prompt_chars = []
            right_prompt_chars = []
            prompt_flag = False
            for i, index in enumerate(index_set):
                ...
                if i not in rands:
                    if not prompt_flag:
                        left_prompt_chars.append(input_ids[index])
                    else:
                        right_prompt_chars.append(input_ids[index])
                    char_coverd_indexes.add(index)
                    mask_length += 1
                else:
                    if prompt_flag:
                        lr_prompts_chars.append([left_prompt_chars, right_prompt_chars])
                        left_prompt_chars = right_prompt_chars
                    prompt_flag = True
                    prompt_index.append(index)
                    prompt_num += 1
            if len(prompt_index) > 0:
                lr_prompts_chars.append([left_prompt_chars, right_prompt_chars])
                prompt_indexes.extend(prompt_index)
        prompt_indexes.append(sep_index)
        assert len(covered_indexes) == len(masked_lms)
        mask_labels = [1 if i in covered_indexes else 0 for i in range(len(input_tokens))]
        char_mask_labels = [1 if i in char_coverd_indexes else 0 for i in range(len(input_tokens))]
        return mask_labels, char_mask_labels, prompt_indexes, lr_prompts_chars

    def torch_mask_tokens(self, inputs: Any, mask_labels: Any, char_mask_labels: Any) -> Tuple[Any, Any]:
        """
        ...
        probability_matrix = char_mask_labels
        label_probability_matrix = mask_labels

        special_tokens_mask = [
            self.tokenizer.get_special_tokens_mask(val, already_has_special_tokens=True) for val in labels.tolist()
        ]
        probability_matrix.masked_fill_(torch.tensor(special_tokens_mask, dtype=torch.bool), value=0.0)
        label_probability_matrix.masked_fill_(torch.tensor(special_tokens_mask, dtype=torch.bool), value=0.0)
        if self.tokenizer._pad_token is not None:
            padding_mask = labels.eq(self.tokenizer.pad_token_id)
            probability_matrix.masked_fill_(padding_mask, value=0.0)
            label_probability_matrix.masked_fill_(padding_mask, value=0.0)

        masked_indices = probability_matrix.bool()
        label_masked_indices = label_probability_matrix.bool()
        labels[~label_masked_indices] = -100  # We only compute loss on masked tokens
